...
@SuppressWarnings("restriction") // GMF Properties are internal
public class CollapseOnClickHandler extends AbstractHandler implements IOnClickHandler {
	...
	public void click(final MouseEvent event) {
		if (getHost() instanceof CollapseHandlePart) {
			final IVisualPart<? extends Node> compartment = CompartmentUtils
					.getCollapsablePart(getHost().getAnchoragesUnmodifiable().keys().iterator().next());
			...
			if (null != compartment) {
				...
				if (editingDomain instanceof TransactionalEditingDomain) {
					SetPropertyCommand setCommand = new SetPropertyCommand((TransactionalEditingDomain) editingDomain,
							"Collapse compartment", new EObjectAdapter(drawerStyle), Properties.ID_COLLAPSED,
							valueToSet);

					editingDomain.getCommandStack().execute(new GMFtoEMFCommandWrapper(setCommand));
					compartment.refreshVisual(); // FIXME Shouldn't be required. The compartment should listen on its
													// DrawerStyle
